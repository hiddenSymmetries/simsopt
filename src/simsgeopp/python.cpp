...
PYBIND11_MODULE(simsgeopp, m) {
    ...
    m.def("boozer_dB_dc", [](PyArray& dB_by_dX, PyArray& dx_dc) {
            int nphi = dB_by_dX.shape(0);
            int ntheta = dB_by_dX.shape(1);
            int ndofs = dx_dc.shape(3);
            PyArray dB_dc = xt::zeros<double>({nphi, ntheta, 3, ndofs});
            for(int i=0; i<nphi; i++){
                for(int j=0; j<ntheta; j++){
                    for (int d = 0; d < 3; ++d) {
                        for (int dd = 0; dd < 3; ++dd) {
                            double dB_by_dX_d = dB_by_dX(i, j, dd, d);
                            auto dx_dc_ptr = &(dx_dc(i, j, dd, 0));
                            auto dB_dc_ptr = &(dB_dc(i, j, d, 0));
                            for (int m = 0; m < ndofs; ++m) {
                                dB_dc_ptr[m] += dB_by_dX_d * dx_dc_ptr[m];
                            }
                        }
                    }
                }
