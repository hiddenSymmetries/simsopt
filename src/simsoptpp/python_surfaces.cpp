...
template <typename T, typename S> void register_common_surface_methods(S &s) {
    s.def("gamma", &T::gamma)
     ...
     /****
     This function takes as input a number, and then uses the plasma normal vectors
     at all quadrature points to extend the surface in question.
     
         Args:
             scale: double
                 Value to use for extending the plasma normal vectors
     ****/
     .def("extend_via_projected_normal", &T::extend_via_projected_normal)
     /****
     This function takes as input a number, and then uses the plasma normal vectors
     at all quadrature points to extend the surface in question. Unlike the 
     extend_via_normal function, this function uses the (R, phi, Z) normal vectors,
     zeros the phi components, and then extends the vectors. This results in a new
     surface with the same toroidal angle locations as the original surface.
     
         Args:
             phi: 1D array
                 Toroidal angle locations
             scale: double
                 Value to use for extending the plasma normal vectors
     ****/
