...
template <typename T, typename S> void register_common_field_methods(S &c) {
    c
     ...
     .def("BdotCurlB", py::overload_cast<>(&T::BdotCurlB), "Returns a `(npoints, 1)` array containing the dot product between the magnetic field B and its curl.")
     .def("CurlbdotGradAbsB", py::overload_cast<>(&T::CurlbdotGradAbsB), "Returns a `(npoints, 1)` array containing the dot product between the magnetic field B and its curl.")
     ...
     .def("BdotCurlB_ref", py::overload_cast<>(&T::BdotCurlB_ref), "As `BdotCurlB`, but returns a reference to the array (this array should be read only).")
     .def("CurlbdotGradAbsB_ref", py::overload_cast<>(&T::CurlbdotGradAbsB_ref), "As `CurlbdotGradAbsB`, but returns a reference to the array (this array should be read only).")
...
void init_magneticfields(py::module_ &m){
    ...
    auto ifield = py::class_<PyInterpolatedField, py_shared_ptr<PyInterpolatedField>, PyMagneticField>(m, "InterpolatedField")
        ...
        .def("estimate_error_CurlbdotGradAbsB", &PyInterpolatedField::estimate_error_CurlbdotGradAbsB)
