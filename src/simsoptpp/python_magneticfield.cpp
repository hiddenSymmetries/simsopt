...
template <typename T, typename S> void register_common_field_methods(S &c) {
    c
     ...
     .def("d3B_by_dXdXdX", py::overload_cast<>(&T::d3B_by_dXdXdX), "Returns a `(npoints, 3, 3, 3, 3)` array containing the third derivative of the magnetic field (in cartesian coordinates). Denoting the indices by `i`, `j`, `k`, `m` and `l`, the result contains  `\\partial_m\\partial_k\\partial_j B_l(x_i)`.")
     ...
     .def("d3B_by_dXdXdX_ref", py::overload_cast<>(&T::d3B_by_dXdXdX_ref), "As `d3B_by_dXdXdX`, but returns a reference to the array (this array should be read only).")
