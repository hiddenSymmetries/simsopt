        ...
        for (int k2 = 0; k2 < numquadpoints_theta; ++k2)
        {
            ...
            for (int m = 0; m <= mpol; ++m)
            {
                for (int n = -ntor; n <= ntor; ++n)
                {
                    if (m == 0 && n < 0)
                        continue;
                    data(k1, k2, 0, counter) = 4 * M_PI * M_PI * (m * (n * nfp) * cos(m * theta - n * nfp * phi) * cos(phi) + m * sin(m * theta - n * nfp * phi) * sin(phi));
                    data(k1, k2, 1, counter) = 4 * M_PI * M_PI * (m * (n * nfp) * cos(m * theta - n * nfp * phi) * sin(phi) - m * sin(m * theta - n * nfp * phi) * cos(phi));
                    data(k1, k2, 2, counter) = 0;
                    counter++;
                }
            }
            if (!stellsym)
            {
                for (int m = 0; m <= mpol; ++m)
                {
                    for (int n = -ntor; n <= ntor; ++n)
                    {
                        if (m == 0 && n <= 0)
                            continue;
                        data(k1, k2, 0, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * m * sin(m * theta - n * nfp * phi) * cos(phi) - m * cos(m * theta - n * nfp * phi) * sin(phi));
                        data(k1, k2, 1, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * m * sin(m * theta - n * nfp * phi) * sin(phi) + m * cos(m * theta - n * nfp * phi) * cos(phi));
                        data(k1, k2, 2, counter) = 0;
                        counter++;
                    }
                }
        ...
        for (int k2 = 0; k2 < numquadpoints_theta; ++k2)
        {
            ...
            for (int m = 0; m <= mpol; ++m)
            {
                for (int n = -ntor; n <= ntor; ++n)
                {
                    if (m == 0 && n < 0)
                        continue;
                    data(k1, k2, 0, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * (-n * nfp) * cos(m * theta - n * nfp * phi) * cos(phi) + 2 * (-n * nfp) * sin(m * theta - n * nfp * phi) * sin(phi) - cos(m * theta - n * nfp * phi) * cos(phi));
                    data(k1, k2, 1, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * (-n * nfp) * cos(m * theta - n * nfp * phi) * sin(phi) - 2 * (-n * nfp) * sin(m * theta - n * nfp * phi) * cos(phi) - cos(m * theta - n * nfp * phi) * sin(phi));
                    data(k1, k2, 2, counter) = 0;
                    counter++;
                }
            }
            if (!stellsym)
            {
                for (int m = 0; m <= mpol; ++m)
                {
                    for (int n = -ntor; n <= ntor; ++n)
                    {
                        if (m == 0 && n <= 0)
                            continue;
                        data(k1, k2, 0, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * (-n * nfp) * sin(m * theta - n * nfp * phi) * cos(phi) - 2 * (-n * nfp) * cos(m * theta - n * nfp * phi) * sin(phi) - sin(m * theta - n * nfp * phi) * cos(phi));
                        data(k1, k2, 1, counter) = 4 * M_PI * M_PI * (-(-n * nfp) * (-n * nfp) * sin(m * theta - n * nfp * phi) * sin(phi) + 2 * (-n * nfp) * cos(m * theta - n * nfp * phi) * cos(phi) - sin(m * theta - n * nfp * phi) * sin(phi));
                        data(k1, k2, 2, counter) = 0;
                        counter++;
                    }
                }
