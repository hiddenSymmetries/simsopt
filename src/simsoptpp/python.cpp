...
PYBIND11_MODULE(simsoptpp, m) {
    ...
    m.def("Pd", &Pd);
    m.def("iterate_over_corners", &iterate_over_corners);
    m.def("Hd_i_prime", &Hd_i_prime);
    m.def("B_direct", &B_direct);
    m.def("Bn_direct", &Bn_direct);
    m.def("gd_i", &gd_i);   
    m.def("Acube", &Acube);    
    m.def("Bn_fromMat", &Bn_fromMat);
    ...
    m.def("GPMO_baseline_shadow", &GPMO_baseline_shadow);
