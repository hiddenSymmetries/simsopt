import numpy as np
from Bgrad import gradr_Bcube, gradr_Bdip
from Bcube import B_direct, Bn_direct, Bn_fromMat, Bdip_direct, Bndip_direct, Bndip_fromMat
...
M = np.array([[0,0,1]])
phithetas = np.array([[np.radian(16),np.radian(134)]])
...
M = np.array([[0,0,1]])
phiTheta = np.array([[np.radian(0),np.radian(90)]])
cubeGrad = []
dipGrad = []
