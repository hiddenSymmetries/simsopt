...
def compute_on_axis_iota(axis, biotsavart):
    """
    ...
    This method was shared by Stuart Hudson and Matt Landreman.

    Args:
        axis: the magnetic axis of a device as a CurveRZFourier.
        biotsavart: the magnetic field in which the axis lies.
    ...
    def tangent_map(phi, x):
        ...
        biotsavart.set_points(out)

        out = out.flatten()
        B = biotsavart.B().flatten()
        dB_by_dX = biotsavart.dB_by_dX().reshape((3, 3))
        B1 = B[0]
        B2 = B[1]
        B3 = B[2]
