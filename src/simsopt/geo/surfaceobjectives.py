...
def BoozerSurfaceResidual(surface, iota, biotsavart, derivatives=0):
    ...
    G = (
        2.0
        * np.pi
        * np.sum(np.abs(biotsavart.coil_currents))
    ...
    dresidual_dc = (
        dB_dc
        - (2 / G)
        * np.sum(B[..., None] * dB_dc, axis=2)[:, :, None, :]
        * tang[..., None]
        - (np.sum(B ** 2, axis=2) / G)[..., None, None] * (dxphi_dc + iota * dxtheta_dc)
    )
    dresidual_diota = -(np.sum(B ** 2, axis=2) / G)[..., None] * xtheta

    dresidual_dc_flattened = dresidual_dc.reshape(
        (nphi * ntheta * 3, dresidual_dc.shape[-1])
    )
    ...
    dB2_dc = 2.0 * np.einsum("ijl,ijlm->ijm", B, dB_dc)
    ...
    d2residual_by_dcdiota = -(1 / G) * (
        dB2_dc[..., None, :] * xtheta[..., :, None] + B2[..., None, None] * dxtheta_dc
    )
    d2residual_by_diotadiota = np.zeros(dresidual_diota.shape)

    d2residual_by_dcdc_flattened = d2residual_by_dcdc.reshape(
        (nphi * ntheta * 3, dresidual_dc.shape[-1], dresidual_dc.shape[-1])
    )
    d2residual_by_dcdiota_flattened = d2residual_by_dcdiota.reshape(
        (nphi * ntheta * 3, dresidual_dc.shape[-1])
    )
    d2residual_by_diotadiota_flattened = d2residual_by_diotadiota.reshape(
        (nphi * ntheta * 3, 1)
    )

    return (
        residual_flattened,
        dresidual_dc_flattened,
        dresidual_diota_flattened,
        d2residual_by_dcdc_flattened,
        d2residual_by_dcdiota_flattened,
        d2residual_by_diotadiota_flattened,
    )
