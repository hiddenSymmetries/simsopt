...
def plot(items, ax=None, show=True, **kwargs):
    """
    Plot curves and/or surfaces together on the same axes. Any keyword
    arguments other than the ones listed here are passed to the
    ``plot()`` method of each item. A particularly useful argument is
    ``engine``, which can be set to ``"matplotlib"``, ``"mayavi"``, or
    ``"plotly"``.

    Args:
        items: A list of objects to plot, consisting of :obj:`~simsopt.geo.curve.Curve` and :obj:`~simsopt.geo.surface.Surface` objects.
