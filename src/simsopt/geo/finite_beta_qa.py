# coding: utf-8
"""Utilities and runners for the finite-beta Boozer QA example."""
...
from .boozersurface import BoozerSurface, FiniteBetaBoozerSurface, SurfaceCurrentFieldProvider
from .curve import curves_to_vtk
from .curveobjectives import CurveLength
from .surfaceobjectives import MU0, Volume, surface_field_nonquasisymmetric_ratio
...
@dataclass
class FiniteBetaQAExampleConfig:
    mode: str = "single-surface-vc"
    nphi: int = 8
    ntheta: int = 8
    ls_max_nfev: int = 30
    qa_maxiter: int = 3
    inner_verbose: bool = False
    write_closure_diagnostics: bool = True
    closure_vc_digits: int = 6
    out_dir: Path | str = Path("output")


def _coerce_mode(requested_mode, legacy_synthetic_mode=None):
    default_mode = "single-surface-vc" if legacy_synthetic_mode is None else ("prescribed" if legacy_synthetic_mode == "1" else "vmec")
    requested_mode = (requested_mode or default_mode).strip().lower()
    if requested_mode in ("single-surface-vc", "vc", "vc-validation"):
        return "single-surface-vc"
    if requested_mode == "self-consistent":
        return "self-consistent"
    return requested_mode


def build_finite_beta_qa_config_from_environment(base_config=None, default_out_dir=None):
    config = base_config or FiniteBetaQAExampleConfig()
    out_dir = Path(default_out_dir if default_out_dir is not None else config.out_dir)
    legacy_synthetic_mode = os.environ.get("SIMSOPT_FINITE_BETA_SYNTHETIC")
    return FiniteBetaQAExampleConfig(
        mode=_coerce_mode(os.environ.get("SIMSOPT_FINITE_BETA_MODE", config.mode), legacy_synthetic_mode),
        nphi=int(os.environ.get("SIMSOPT_FINITE_BETA_NPHI", str(config.nphi))),
        ntheta=int(os.environ.get("SIMSOPT_FINITE_BETA_NTHETA", str(config.ntheta))),
        ls_max_nfev=int(os.environ.get("SIMSOPT_FINITE_BETA_MAX_NFEV", str(config.ls_max_nfev))),
        qa_maxiter=int(os.environ.get("SIMSOPT_FINITE_BETA_QA_MAXITER", str(config.qa_maxiter))),
        inner_verbose=os.environ.get("SIMSOPT_FINITE_BETA_INNER_VERBOSE", "1" if config.inner_verbose else "0").strip().lower() in ("1", "true", "yes", "on"),
        write_closure_diagnostics=os.environ.get("SIMSOPT_FINITE_BETA_WRITE_CLOSURE_DIAGNOSTICS", "1" if config.write_closure_diagnostics else "0").strip().lower() in ("1", "true", "yes", "on"),
        closure_vc_digits=int(os.environ.get("SIMSOPT_FINITE_BETA_COMPARE_VC_DIGITS", os.environ.get("SIMSOPT_FINITE_BETA_VC_DIGITS", str(config.closure_vc_digits)))),
        out_dir=Path(os.environ.get("SIMSOPT_FINITE_BETA_OUT_DIR", str(out_dir))),
    )


def _configure_runtime(config):
    global OUT_DIR, mode, nphi, ntheta, ls_max_nfev, qa_maxiter
    global inner_verbose, write_closure_diagnostics, closure_vc_digits

    OUT_DIR = str(Path(config.out_dir))
    if not OUT_DIR.endswith(os.sep):
        OUT_DIR = OUT_DIR + os.sep
    os.makedirs(OUT_DIR, exist_ok=True)

    mode = _coerce_mode(config.mode, os.environ.get("SIMSOPT_FINITE_BETA_SYNTHETIC"))
    nphi = int(config.nphi)
    ntheta = int(config.ntheta)
    ls_max_nfev = int(config.ls_max_nfev)
    qa_maxiter = int(config.qa_maxiter)
    inner_verbose = bool(config.inner_verbose)
    write_closure_diagnostics = bool(config.write_closure_diagnostics)
...
def run_direct_self_consistent_continuation(
    ...
    zero_toroidal_current = _env_flag("SIMSOPT_FINITE_BETA_DIRECT_ZERO_TOROIDAL_CURRENT", default=False)
    max_subdivisions = max(0, int(os.environ.get("SIMSOPT_FINITE_BETA_SELFCONSISTENT_MAX_SUBDIVISIONS", "8")))
    max_iota_jump = float(os.environ.get("SIMSOPT_FINITE_BETA_SELFCONSISTENT_MAX_IOTA_JUMP", "0.05"))
    initial_pressure_jump = float(finite_beta.pressure_jump)
    target_pressure_jump = float(pressure_jump)
    if continuation_steps == 1 or abs(target_pressure_jump - initial_pressure_jump) == 0.0:
        pressure_schedule = np.asarray([target_pressure_jump])
    else:
        pressure_schedule = np.linspace(initial_pressure_jump, target_pressure_jump, continuation_steps)

    current_state = {
        'pressure_jump': initial_pressure_jump,
        'iota': float(iota),
        'G': float(G),
        'I': 0.0 if zero_toroidal_current else float(I),
        'current_potential': np.asarray(current_potential).copy(),
    }
    stage_history = []

    def solve_step(base_state, step_pressure):
        finite_beta.pressure_jump = float(step_pressure)
        result = finite_beta.run_code(
            iota=float(base_state['iota']),
            G=float(base_state['G']),
            I=0.0 if zero_toroidal_current else float(base_state['I']),
            current_potential=np.asarray(base_state['current_potential']).copy(),
        ...
        state = {
            ...
            'G': float(result['G']),
            'I': 0.0 if zero_toroidal_current else float(result['I']),
            'current_potential': np.asarray(result['current_potential']).copy(),
        }
        return result, state

    def accept_step(base_state, candidate_result, candidate_state):
        if not bool(candidate_result['success']):
            return False
        return abs(float(candidate_state['iota']) - float(base_state['iota'])) <= max_iota_jump

    def advance_with_subdivision(base_state, step_pressure, depth):
        candidate_result, candidate_state = solve_step(base_state, step_pressure)
        if accept_step(base_state, candidate_result, candidate_state) or depth >= max_subdivisions:
            if depth >= max_subdivisions and not accept_step(base_state, candidate_result, candidate_state):
                raise RuntimeError(
                    f"Direct self-consistent continuation could not preserve branch continuity at pressure_jump={float(step_pressure):.6e}. "
                    f"last_success={bool(candidate_result['success'])}, "
                    f"iota_jump={abs(float(candidate_state['iota']) - float(base_state['iota'])):.6e}, "
                    f"threshold={max_iota_jump:.6e}"
                )
            return candidate_result, candidate_state

        midpoint = 0.5 * (float(base_state['pressure_jump']) + float(step_pressure))
        midpoint_result, midpoint_state = advance_with_subdivision(base_state, midpoint, depth + 1)
        stage_history.append({
            'pressure_jump': float(midpoint),
            'success': bool(midpoint_result['success']),
            'nfev': int(midpoint_result['iter']),
            'weighted_residual_norm': float(midpoint_result['residual_norm']),
            'iota': float(midpoint_result['iota']),
            'G': float(midpoint_result['G']),
            'I': float(midpoint_result['I']),
        })
        return advance_with_subdivision(midpoint_state, step_pressure, depth + 1)

    result = None
    for step_pressure in pressure_schedule:
        if abs(float(step_pressure) - float(current_state['pressure_jump'])) < 1e-15:
            continue
        result, current_state = advance_with_subdivision(current_state, step_pressure, 0)
        stage_history.append({
            'pressure_jump': float(step_pressure),
    ...
    if result is None:
        result, current_state = solve_step(current_state, current_state['pressure_jump'])
...
def vmec_boundary_triggers_theta_flip(boundary_rz):
    rtest = sum(boundary_rz.get_rc(1, n) for n in range(-boundary_rz.ntor, boundary_rz.ntor + 1))
    ztest = sum(boundary_rz.get_zs(1, n) for n in range(-boundary_rz.ntor, boundary_rz.ntor + 1))
    return (rtest * ztest) < 0.0


def preflip_vmec_boundary(boundary_rz):
    flipped = boundary_rz.copy()
    for m in range(1, flipped.mpol + 1):
        rc_factor = -1.0 if (m % 2 == 1) else 1.0
        zs_factor = 1.0 if (m % 2 == 1) else -1.0
        for n in range(-flipped.ntor, flipped.ntor + 1):
            flipped.set_rc(m, n, rc_factor * flipped.get_rc(m, n))
            flipped.set_zs(m, n, zs_factor * flipped.get_zs(m, n))
    return flipped


def build_vmec_total_current_profile(nodes, family="smoothstep", alpha=1.0, beta=1.0):
    nodes = np.asarray(nodes, dtype=float)
    if family == "smoothstep":
        values = 3.0 * nodes**2 - 2.0 * nodes**3
    elif family in ("beta", "beta_cdf"):
        s_dense = np.linspace(0.0, 1.0, max(2001, 256 * max(len(nodes) - 1, 1) + 1))
        derivative = np.power(np.clip(s_dense, 0.0, 1.0), float(alpha)) * np.power(np.clip(1.0 - s_dense, 0.0, 1.0), float(beta))
        derivative[0] = 0.0
        derivative[-1] = 0.0
        cumulative = np.zeros_like(s_dense)
        cumulative[1:] = np.cumsum(0.5 * (derivative[1:] + derivative[:-1]) * np.diff(s_dense))
        if cumulative[-1] <= 0.0:
            raise ValueError(f"Invalid VMEC current-profile parameters alpha={alpha}, beta={beta}")
        cumulative /= cumulative[-1]
        values = np.interp(nodes, s_dense, cumulative)
    else:
        raise ValueError(f"Unsupported VMEC current-profile family: {family}")
...
def parse_vmec_current_profile_candidates(nodes):
    scan_spec = os.environ.get("SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_SCAN", "").strip()
    if scan_spec:
        raw_candidates = [token.strip() for token in scan_spec.split(";") if token.strip()]
    else:
        family = os.environ.get("SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_FAMILY", "smoothstep").strip().lower()
        if family in ("beta", "beta_cdf"):
            alpha = float(os.environ.get("SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_ALPHA", "1.0"))
            beta = float(os.environ.get("SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_BETA", "1.0"))
            raw_candidates = [f"beta_cdf:{alpha}:{beta}"]
        else:
            raw_candidates = [family]
    ...
    for token in raw_candidates:
        ...
        if family == "smoothstep":
            ...
            if len(parts) != 3:
                raise ValueError(
                    "VMEC beta-cdf current profiles must use 'beta_cdf:<alpha>:<beta>' in "
                    "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_SCAN"
                )
            alpha = float(parts[1])
            beta = float(parts[2])
            family = "beta_cdf"
            name = f"beta_cdf_a{alpha:.6g}_b{beta:.6g}"
        else:
            raise ValueError(f"Unsupported VMEC current-profile candidate '{token}'")
        candidates.append(
            {
                "name": name,
                "family": family,
                "alpha": alpha,
                "beta": beta,
                "values": build_vmec_total_current_profile(nodes, family=family, alpha=alpha, beta=beta),
            }
        )
    return candidates


def score_vmec_current_profile_candidate(vmec_run, direct_iota, direct_toroidal_current):
    edge_iota = float(vmec_run.iota_edge())
    mean_iota = float(vmec_run.mean_iota())
    current_rel_error = float((float(vmec_run.wout.ctor) - float(direct_toroidal_current)) / max(abs(float(direct_toroidal_current)), 1e-30))
    return (
        abs(edge_iota - float(direct_iota)),
        abs(mean_iota - float(direct_iota)),
        abs(current_rel_error),
    )


def normalized_boozer_I_to_toroidal_current(I):
    r"""
    Convert the direct single-surface Boozer ``I`` coefficient to total
    toroidal current in amperes.

    The direct finite-beta workflow uses surface derivatives with respect to the
    normalized angles in ``[0, 1)``, so its covariant Boozer coefficients are
    larger than the standard VMEC/radian-angle coefficients by a factor of
    :math:`2\pi`. Therefore, the physical current is ``I / mu0``, not
    ``(2 pi / mu0) I``.
    """
    return float(float(I) / MU0)
...
def resolve_vmec_current_target_from_direct_state(direct_I):
    explicit_current = os.environ.get("SIMSOPT_FINITE_BETA_VMEC_TOROIDAL_CURRENT_A")
    if explicit_current is not None:
        return float(explicit_current), "explicit_total_current"

    if _env_flag("SIMSOPT_FINITE_BETA_VMEC_ALLOW_UNDERDETERMINED_CURRENT", default=False):
        return normalized_boozer_I_to_toroidal_current(direct_I), "derived_direct_I_override"

    raise ValueError(
        "Current-constrained VMEC export from the direct single-surface finite-beta state is physically "
        "underdetermined: the one-surface model does not determine a unique volume toroidal-current target/profile. "
        "Use SIMSOPT_FINITE_BETA_VMEC_CONSTRAINT_MODE=iota, or provide the missing physics explicitly via "
        "SIMSOPT_FINITE_BETA_VMEC_TOROIDAL_CURRENT_A together with an explicit current-profile specification, or set "
        "SIMSOPT_FINITE_BETA_VMEC_ALLOW_UNDERDETERMINED_CURRENT=1 to reproduce the old heuristic behavior."
    )
...
def export_and_benchmark_vmec_from_surface(
    ...
    vmec_boundary_preflipped = False
    if vmec_boundary_triggers_theta_flip(boundary_rz):
        boundary_rz = preflip_vmec_boundary(boundary_rz)
        vmec_boundary_preflipped = True
    beta_percent = 100.0 * float(target_plasma_beta)
    beta_label = f"{beta_percent:.3f}".rstrip("0").rstrip(".").replace(".", "p")
    vmec_input_name = os.environ.get(
        "SIMSOPT_FINITE_BETA_VMEC_INPUT_NAME",
        f"input.boozerQA_finitebeta_direct_beta{beta_label}",
    )
    ...
    if constraint_mode in ("stellarator", "zero-current", "zero_current"):
        constraint_mode = "iota"
    ...
    current_profile_explicit = any(
        os.environ.get(name) is not None
        for name in (
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_SCAN",
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_FAMILY",
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_ALPHA",
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_BETA",
        )
    )
    allow_underdetermined_current = _env_flag("SIMSOPT_FINITE_BETA_VMEC_ALLOW_UNDERDETERMINED_CURRENT", default=False)
    if constraint_mode == "current":
        direct_toroidal_current, vmec_current_target_source = resolve_vmec_current_target_from_direct_state(direct_I)
    else:
        direct_toroidal_current = normalized_boozer_I_to_toroidal_current(direct_I)
        vmec_current_target_source = "zero_current_stellarator"
    vmec_iota_target = float(direct_iota)
    current_continuation_scales = [
        float(value.strip())
        for value in os.environ.get(
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_SCALES",
            "0.05,0.1,0.2,0.35,0.5,0.75,1.0",
        ).split(",")
        if value.strip()
    ]
    current_profile_nodes = np.linspace(0.0, 1.0, int(os.environ.get("SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_NODES", "9")))
    current_profile_candidates = parse_vmec_current_profile_candidates(current_profile_nodes) if constraint_mode == "current" else []
    if constraint_mode == "current" and not current_profile_explicit and not allow_underdetermined_current:
        raise ValueError(
            "Current-constrained VMEC export requires an explicit radial current profile because the direct single-surface "
            "state does not determine one. Set SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_FAMILY or "
            "SIMSOPT_FINITE_BETA_VMEC_CURRENT_PROFILE_SCAN, or set "
            "SIMSOPT_FINITE_BETA_VMEC_ALLOW_UNDERDETERMINED_CURRENT=1 to reuse the old smoothstep heuristic."
        )
    if constraint_mode == "current" and len(current_profile_candidates) != 1 and not allow_underdetermined_current:
        raise ValueError(
            "Current-constrained VMEC export with multiple current-profile candidates is a heuristic scan, not a physics-unique "
            "embedding. Provide exactly one explicit current profile, or set "
            "SIMSOPT_FINITE_BETA_VMEC_ALLOW_UNDERDETERMINED_CURRENT=1 to allow heuristic candidate selection."
        )

    def build_vmec_object():
        return Vmec(None, verbose=False, nphi=max(nphi_benchmark, 32), ntheta=max(ntheta_benchmark, 32), range_surface="field period")

    def configure_vmec_equilibrium(vmec_obj, axis_pressure_value, phiedge_value, direct_iota_value, current_scale=1.0, current_profile_values=None):
        ...
        if constraint_mode == "iota":
            ...
            vmec_obj.indata.curtor = 0.0
            vmec_obj.indata.piota_type = "power_series"
            vmec_obj.iota_profile = ProfilePolynomial([vmec_iota_target])
        elif constraint_mode == "current":
            if current_profile_values is None:
                raise ValueError("Current-constrained VMEC export requires current_profile_values")
            vmec_obj.indata.ncurr = 1
            vmec_obj.indata.pcurr_type = "cubic_spline_i"
            vmec_obj.current_profile = ProfileSpline(
                current_profile_nodes,
                float(current_scale) * direct_toroidal_current * np.asarray(current_profile_values, dtype=float),
            )
        else:
            vmec_obj.indata.ncurr = 0
            vmec_obj.indata.curtor = 0.0
    ...
    def calibrate_phiedge(current_profile_values=None):
        calibration_vmec = build_vmec_object()
        calibration_current_scale = 1.0 if constraint_mode == "current" else 0.0
        configure_vmec_equilibrium(
            calibration_vmec,
            axis_pressure_value=0.0,
            phiedge_value=1.0,
            direct_iota_value=direct_iota,
            current_scale=calibration_current_scale,
            current_profile_values=current_profile_values,
        )
        ...
        return float(np.sqrt(reference_magnetic_pressure / max(calibration_magnetic_pressure, 1e-30)))

    def run_current_mode(axis_pressure_value, phiedge_value, current_profile_values):
        vmec_run = None
        last_error = None
        vmec_current_scale_used_local = 0.0
        for scale in current_continuation_scales:
            vmec_trial = build_vmec_object()
            configure_vmec_equilibrium(
                vmec_trial,
                axis_pressure_value=axis_pressure_value * float(scale),
                phiedge_value=phiedge_value,
                direct_iota_value=direct_iota,
                current_scale=scale,
                current_profile_values=current_profile_values,
            )
            try:
                vmec_trial.run()
            except ObjectiveFailure as err:
                last_error = err
                break
            vmec_run = vmec_trial
            vmec_current_scale_used_local = float(scale)

        if vmec_run is None or vmec_current_scale_used_local < 1.0 - 1e-12:
            raise RuntimeError(
                "Current-constrained VMEC continuation did not reach full direct current. "
                f"Reached scale={vmec_current_scale_used_local:.3f} of {direct_toroidal_current:.6e} A"
                + (f"; last_error={last_error}" if last_error is not None else "")
            )

        return vmec_run, vmec_current_scale_used_local

    def run_iota_mode(axis_pressure_value, phiedge_value):
        vmec = build_vmec_object()
        configure_vmec_equilibrium(vmec, axis_pressure_value=axis_pressure_value, phiedge_value=phiedge_value, direct_iota_value=direct_iota)
        vmec.run()
        return vmec, 1.0

    def run_calibrated_current_candidate(candidate, axis_pressure_initial, beta_iterations):
        axis_pressure_value = float(axis_pressure_initial)
        phiedge_value = calibrate_phiedge(candidate["values"])
        vmec_run_local, vmec_current_scale_used_local = run_current_mode(axis_pressure_value, phiedge_value, candidate["values"])
        for _ in range(max(0, beta_iterations)):
            betatotal = float(getattr(vmec_run_local.wout, "betatotal", np.nan))
            if not np.isfinite(betatotal) or betatotal <= 0.0:
                break
            pressure_scale = float(target_plasma_beta) / betatotal
            if abs(pressure_scale - 1.0) < 5e-3:
                break
            axis_pressure_value *= pressure_scale
            vmec_run_local, vmec_current_scale_used_local = run_current_mode(axis_pressure_value, phiedge_value, candidate["values"])
        return {
            "candidate": candidate,
            "vmec_run": vmec_run_local,
            "vmec_current_scale_used": float(vmec_current_scale_used_local),
            "axis_pressure": float(axis_pressure_value),
            "phiedge": float(phiedge_value),
            "score": score_vmec_current_profile_candidate(vmec_run_local, direct_iota, direct_toroidal_current),
        }

    axis_pressure = float(max(2.0 * target_plasma_beta * reference_magnetic_pressure, 0.0))
    vmec_axis_pressure_initial = float(axis_pressure)
    beta_iterations = int(os.environ.get("SIMSOPT_FINITE_BETA_VMEC_BETA_CALIBRATION_STEPS", "2"))
    selected_current_profile = None
    if constraint_mode == "current":
        candidate_results = []
        candidate_errors = []
        for candidate in current_profile_candidates:
            try:
                candidate_result = run_calibrated_current_candidate(candidate, axis_pressure, beta_iterations)
            except RuntimeError as err:
                candidate_errors.append(f"{candidate['name']}: {err}")
                print(f"VMEC current-profile candidate {candidate['name']} failed: {err}")
                continue
            candidate_results.append(candidate_result)
            candidate_vmec_run = candidate_result["vmec_run"]
            print(
                "VMEC current-profile candidate "
                f"{candidate['name']} edge_iota={float(candidate_vmec_run.iota_edge()):.6e}, "
                f"mean_iota={float(candidate_vmec_run.mean_iota()):.6e}, "
                f"betatotal={float(getattr(candidate_vmec_run.wout, 'betatotal', np.nan)):.6e}"
            )
        if len(candidate_results) == 0:
            raise RuntimeError(
                "No current-profile candidate reached a converged current-constrained VMEC state. "
                + ("; ".join(candidate_errors) if candidate_errors else "")
            )
        best_candidate_result = min(candidate_results, key=lambda item: item["score"])
        selected_current_profile = best_candidate_result["candidate"]
        vmec_run = best_candidate_result["vmec_run"]
        vmec_current_scale_used = best_candidate_result["vmec_current_scale_used"]
        axis_pressure = best_candidate_result["axis_pressure"]
        phiedge_scale = best_candidate_result["phiedge"]
        print(f"Selected VMEC current-profile candidate {selected_current_profile['name']}")
    else:
        phiedge_scale = calibrate_phiedge()
        vmec_run, vmec_current_scale_used = run_iota_mode(axis_pressure, phiedge_scale)

    vmec_run.write_input(str(vmec_input_path))
    ...
    selected_current_profile_name = selected_current_profile["name"] if selected_current_profile is not None else ""
    selected_current_profile_family = selected_current_profile["family"] if selected_current_profile is not None else ""
    selected_current_profile_alpha = float(selected_current_profile["alpha"]) if selected_current_profile is not None else np.nan
    selected_current_profile_beta = float(selected_current_profile["beta"]) if selected_current_profile is not None else np.nan
    benchmark_summary = {
        ...
        'vmec_toroidal_current_target': 0.0 if constraint_mode != 'current' else float(direct_toroidal_current),
        'vmec_constraint_mode': constraint_mode,
        'vmec_current_target_source': vmec_current_target_source,
        'vmec_boundary_preflipped': int(vmec_boundary_preflipped),
        'vmec_iota_target_input': float(vmec_iota_target),
        'vmec_current_scale_used': float(vmec_current_scale_used),
        'vmec_current_profile_name': selected_current_profile_name,
        'vmec_current_profile_family': selected_current_profile_family,
        'vmec_current_profile_alpha': selected_current_profile_alpha,
        'vmec_current_profile_beta': selected_current_profile_beta,
        'vmec_current_profile_candidate_count': int(len(current_profile_candidates)),
        'vmec_axis_pressure_initial': float(vmec_axis_pressure_initial),
...
def _run_selected_mode():
    if mode == "self-consistent":
        ...
        couple_surface_during_continuation = _env_flag(
            "SIMSOPT_FINITE_BETA_SELFCONSISTENT_COUPLE_SURFACE_CONTINUATION",
            default=False,
        )
        ...
        current_I = 0.0
        current_potential = initial_potential
        continuation_optimize_surface = optimize_surface and (
            continuation_steps == 1 or couple_surface_during_continuation
        )
        for step_index, step_pressure in enumerate(continuation_pressures, start=1):
            result = run_direct_self_consistent_continuation(
                ...
                optimize_surface=continuation_optimize_surface,
        ...
        if optimize_surface and not continuation_optimize_surface:
            fixed_surface_result = dict(result)
            fixed_surface_result['current_potential'] = np.asarray(result['current_potential']).copy()
            fixed_surface_history = list(fixed_surface_result.get('continuation_history', []))
            fixed_surface_x = surface.x.copy()
            print("Running final coupled surface relaxation at target pressure from the fixed-surface continuation state")
            result = run_direct_self_consistent_continuation(
                finite_beta,
                field_provider,
                iota=current_iota,
            ...
            result = dict(result)
            result['continuation_history'] = fixed_surface_history + list(result.get('continuation_history', []))
            print(
                f"Final surface relaxation: success={result['success']}, iota={result['iota']:.6e}, "
                f"I={result['I']:.6e}, ||r||={result['residual_norm']:.6e}"
            )
            if not bool(result['success']):
                print("Final coupled surface relaxation did not converge; retaining the fixed-surface continuation state")
                surface.x = fixed_surface_x
                result = fixed_surface_result
            current_iota = result['iota']
            current_G = result['G']
            current_I = result['I']
            current_potential = result['current_potential']
        ...
        test_dir = REPO_ROOT / "tests" / "test_files"
...
def run_finite_beta_qa_example(config=None):
    runtime_config = config or FiniteBetaQAExampleConfig()
    _configure_runtime(runtime_config)
...
__all__ = [
    "FiniteBetaQAExampleConfig",
    "build_finite_beta_qa_config_from_environment",
    "run_finite_beta_qa_example",
]
