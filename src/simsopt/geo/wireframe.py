...
class ToroidalWireframe(object):
    ...
    def to_vtk(self, filename, extent='torus', extra_node_data=None, \
        ...
        hdrStrs = ['Contains data for a wireframe grid generated by SIMSOPT. ' \
                       + 'The wireframe ', \
                   'consists of a set of interconnected current-carrying ' \
                       + 'line segments.', '']

        if extent=='half period':
            hdrStrs.append('In this file, a single half period is represented.')
        elif extent=='field period':
            hdrStrs.append('In this file, a single field period is ' \
                            + 'represented.')
        elif extent=='full torus':
            hdrStrs.append('In this file, a full torus is represented.')

        hdrStrs += ['', \
             'The following quantities are available for each segment:', \
             '    current: Current [A] flowing in each segment. Positive ' \
                 + 'currents ', \
             '        flow in the positive toroidal/poloidal direction.', \
             '    constrained: 1 if segment is constrained to carry no ' \
                 + 'current; 0 otherwise.', \
             '    constrained_exp: 1 if segment explicitly constrained, i.e. ' \
                 + 'set by the user ', \
             '        to carry no current.', \
             '    constrained_imp: 1 if segment is implicitly constrained, '
                 + 'i.e. it can carry no ', \
             '        current due to neighboring segments being constrained.']
