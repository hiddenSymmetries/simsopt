...
def vmec_fieldlines(vs, s, alpha, theta1d=None, phi1d=None, phi_center=0, plot=False, show=True):
    ...
    def residual(theta_v, phi0, theta_p_target, xm, xn, lmns):
        """
        This function is used for computing the value of theta_vmec that
        gives a desired theta_pest.
        """
        
        return theta_p_target - (theta_v + np.sum(lmns[:, None] * np.sin(xm[:, None] * theta_v - xn[:, None] * phi0), axis=0))
    ...
    for js in range(ns):
        for jalpha in range(nalpha):
            ...
            solution = newton(residual, x0 = theta_guess - 0.5, args=(phi[js, jalpha, :], theta_pest[js, jalpha, :], xm, xn, lmns[js]), x1 = theta_guess + 0.5)
            #pdb.set_trace()
