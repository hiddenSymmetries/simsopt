    ...
    if printinput == True:
        ...
        for i in range(len(rbc)):
            if rbc[i] !=0 or zbs[i] != 0:
                print(f"RBC({int(xn[i]/nfp):2d},{int(xm[i]):2d})={rbc[i]:24.15e}, ZBS({int(xn[i]/nfp):2d},{int(xm[i]):2d})={zbs[i]:24.15e}") 
...
def bbooz(fname,sarr,ns):
    ...
    for js in range(len(sarr)):
        ...
        cplot = plt.contour(phis2D-phimin,thetas2D,B[js,:,:], nconts, linewidths=1.0,cmap=cm.plasma)

        plt.colorbar()
        axs.append(plt.gca())
        plt.gca().tick_params(direction='in', length=0)
        color = (0.9, 0.9, 0.9)
        plt.text(0.05, 6.13, f"|B| @ s={s}", ha='left', va='top',
                fontsize=9,
...
file = 'wout_default_000_000000.nc'
...
main(file,
    printinfo,pltdata,pltconts,plt3d,save)
