...
permanent magnet configurations for the Landreman/Paul QH design with
basic cartesian brick magnets.

The script should be run as:
    mpirun -n 1 python permanent_magnet_QH.py
...
OUT_DIR = 'permanent_magnet_QH_output_nphi' + str(nphi) + '/'
os.makedirs(OUT_DIR, exist_ok=True)

# initialize the coils
base_curves, curves, coils = initialize_coils('qh', TEST_DIR, OUT_DIR, s)
...
s, bs = coil_optimization(s, bs, base_curves, curves, OUT_DIR, s_plot, 'qh')
...
pm_opt = PermanentMagnetGrid(
    s, coil_offset=coff, Nx=100, plasma_offset=poff,
    Bn=Bnormal, surface_flag='wout', coordinate_flag='cartesian',
    filename=surface_filename,
)

kwargs = initialize_default_kwargs('GPMO')
algorithm = 'baseline'
kwargs['K'] = 20000  # Number of magnets to place... 50000 for a full run perhaps
#kwargs['verbose'] = True
...
#kwargs['Nadjacent'] = 20  # Number of neighbor dipoles to consider as adjacent
#kwargs['nhistory'] = 200  # Number of neighbor dipoles to consider as adjacent
