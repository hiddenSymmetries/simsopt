...
# iters = 70
iters = 1 # doesn't vary grid distance, just uses premade MUSE grid
...
for it in range(iters):
    ...
    out_str = f"bulkMUSE_RS/exact_poff{poff[it]}"
    ...
    b_dipole._toVTK(out_dir / "Dipole_Fields", pm_opt.ox, pm_opt.oy, pm_opt.oz)
    ...
    assert pm_comp.ox == pm_opt.ox
    assert pm_comp.oy == pm_opt.oy
    assert pm_comp.oz == pm_opt.oz

    b_comp.set_points(s_plot.gamma().reshape((-1, 3)))
    b_comp._toVTK(out_dir / "magnet_fields", pm_comp.ox, pm_comp.oy, pm_comp.oz)
