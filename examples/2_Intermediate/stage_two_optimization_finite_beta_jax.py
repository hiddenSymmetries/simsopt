...
B_{External} dot n is nonzero. A JAX virtual casing calculation is used
to compute this target B_{External} dot n.

The objective is given by

    J = (1/2) integral |B_{BiotSavart} dot n - B_{External} dot n|^2 ds
        + LENGTH_PENALTY * sum 1/2(CurveLength - L0)^2
"""
...
from simsopt.mhd import VirtualCasingJax, VmecJax
...
# see if the virtual casing output file already exists. If so, load
...
if os.path.isfile(vc_filename):
    ...
    vc = VirtualCasingJax.load(vc_filename)
else:
    print('Running the JAX virtual casing calculation')
    vc = VirtualCasingJax.from_vmec(vmec_file, src_nphi=vc_src_nphi, trgt_nphi=nphi, trgt_ntheta=ntheta)

# Initialize the boundary magnetic surface:
s = SurfaceRZFourier.from_wout(vmec_file, range="half period", nphi=nphi, ntheta=ntheta)
total_current = VmecJax(vmec_file).external_current() / (2 * s.nfp)
...
def fun(dofs):
    ...
    outstr = f"J={J:.1e}, Jf={jf:.1e}, mean(|B.n|)={BdotN_mean:.1e}, max(|B.n|)={BdotN_max:.1e}"
