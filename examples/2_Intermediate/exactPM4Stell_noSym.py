...
'''
    ...
    magnets if they are exactly equal and opposite).
...
bn_plasma = bnormal_obj_ncsx.bnormal_grid(nphi, ntheta, 'full torus')
...
if True:
    ...
    tr2 = time.time()
    print('points to vtk took t = ', tr2-tr, ' s')
    ...
    Bnormal_total = Bnormal_plasma + Bnormal_coils + Bnormal_pms 
    pointData = {"B_N": Bnormal_plasma[:, :, None]}
    s_plot.to_vtk(out_dir / "Bnormal_plasma", extra_data=pointData)
    pointData = {"B_N": Bnormal_pms[:, :, None]}
    s_plot.to_vtk(out_dir / "Bnormal_pms", extra_data=pointData)
