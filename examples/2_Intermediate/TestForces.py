...
out_dir = Path("TestForces_Output")
out_dir.mkdir(parents=True, exist_ok=True)

# Read in the plasma equilibrium file
TEST_DIR = (Path(__file__).parent / ".." / ".." / "tests" / "test_files").resolve()

TestFiles = ['CubeTestForces.focus', 'TwoDipoleTestForces.focus', 'TwoDipoleTestForcesPerpendicular.focus', 'SquareTestForces.focus']

for test_file in TestFiles:
    famus_filename = TEST_DIR /  "ForceTestFiles" /test_file
    ...
    mag_data = FocusData(str(famus_filename), downsample=1)
    ...
    pm_grid = PermanentMagnetGrid.geo_setup_from_famus(s, Bnormal, str(famus_filename))
    ...
    output_name = test_file.split('.')[0]
    b_dipole._toVTK(out_dir / output_name)

curves_file = out_dir / "curves_init.vtu"
