...
coff = 1.9  # PSC grid will be initialized between 1 m and 2 m from plasma
...
out_str = "QA_psc_restricted/"
try:
    shutil.rmtree(out_str)
except OSError as e:
    print("Error: %s - %s." % (e.filename, e.strerror))
out_dir = Path("QA_psc_restricted")
...
x0 = np.ravel(np.array([alphas_initial, deltas_initial]))
for k, eps in enumerate(epsilons):
    ...
    psc_array.plot_curves('final_eps_{0:.3e}'.format(eps) + '_N{0:d}'.format(psc_array.num_psc) + '_')
    currents = []
    for i in range(psc_array.num_psc):
        currents.append(Current(psc_array.I[i]))
    all_coils = coils_via_symmetries(
    ...
    B_PSC = BiotSavart(all_coils)
    B_tot = bs + B_PSC
    B_tot.save('B_total_eps_{0:.3e}'.format(eps) + '_N{0:d}'.format(psc_array.num_psc) + '.json')
    make_Bnormal_plots(B_tot, s_plot, out_dir, 'PSC_and_TF_final_eps_{0:.3e}'.format(eps) + '_N{0:d}'.format(psc_array.num_psc), B_axis)
    ...
    #     bounds_inds = np.hstack((big_I_inds, big_I_inds + psc_array.num_psc))
    #     opt_bounds = np.array(opt_bounds)[bounds_inds, :]
        # opt_bounds = tuple(map(tuple, opt_bounds))
    # else:
    #     print('STLSQ converged, breaking out of loop')
    #     break
    BdotN2_list.append(psc_array.BdotN2_list[-1])
    Bn_list.append(psc_array.Bn_list[-1])

    kwargs_manual["alphas"] = alphas_initial
    kwargs_manual["deltas"] = deltas_initial
    # try:
    psc_array = PSCgrid.geo_setup_manual(
        psc_array.grid_xyz, psc_array.R, **kwargs_manual
    )
    # except TypeError:
...
color = 'b'
ax1.set_xlabel(r'Bounds on $(\alpha_i, \delta_i)$')
ax1.set_ylabel(r'$f_B$', color=color)
ax1.semilogy(epsilons * 180.0 / np.pi, BdotN2_list, color=color, label=r'$f_B$')
ax1.semilogy(epsilons * 180.0 / np.pi, Bn_list, color + '--', label=r'$b_n$')
ax1.semilogy(epsilons * 180.0 / np.pi, 0.005 * np.ones(len(Bn_list)), 'm--', label=r'$b_n$, benchmark')
ax1.plot(epsilons * 180.0 / np.pi, fB_TF * np.ones(len(BdotN2_list)), 'k--', label=r'$f_B$, TF coils only')
ax1.grid()
