...
from simsopt.geo.wp_grid import WPgrid
...
out_str = "QA_wp_output/"
out_dir = Path("QA_wp_output")
...
print('Number of WP locations = ', len(wp_array.grid_xyz))

currents = []
for i in range(wp_array.num_wp):
    currents.append(Current(wp_array.I[i]))
all_coils = coils_via_symmetries(
    wp_array.curves, currents, nfp=wp_array.nfp, stellsym=wp_array.stellsym
)
B_WP = BiotSavart(all_coils)
# Plot initial errors from only the WPs, and then together with the TF coils
make_Bnormal_plots(B_WP, s_plot, out_dir, "biot_savart_WP_initial", B_axis)
...
bs.set_points(s.gamma().reshape(-1, 3))
Bnormal = np.sum(bs.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)
print('fB only TF direct = ', np.sum(Bnormal.reshape(-1) ** 2 * wp_array.grid_normalization ** 2
                                    ) / (2 * B_axis ** 2 * s.area()))
make_Bnormal_plots(B_WP, s_plot, out_dir, "biot_savart_WP_initial", B_axis)
fB = SquaredFlux(s, B_WP, np.zeros((nphi, ntheta))).J()
print(fB/ (B_axis ** 2 * s.area()))
fB = SquaredFlux(s, B_WP + bs, np.zeros((nphi, ntheta))).J()
print('fB with both, before opt = ', fB / (B_axis ** 2 * s.area()))
fB = SquaredFlux(s, B_WP, -Bnormal).J()
print('fB with both (minus sign), before opt = ', fB / (B_axis ** 2 * s.area()))
Bnormal_WP = np.sum(B_WP.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)
fB = SquaredFlux(s, bs, -Bnormal_WP).J()
print('fB with both, before opt = ', fB / (B_axis ** 2 * s.area()))
...
# print(x_opt.hess_inv)
    ...
    # Initialize new WP array with coils only at the remaining locations
...
make_Bnormal_plots(B_WP, s_plot, out_dir, "WP_final", B_axis)
make_Bnormal_plots(bs + B_WP, s_plot, out_dir, "WP_and_TF_final", B_axis)
print('end')
