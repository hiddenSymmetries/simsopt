...
FORCE_WEIGHT = Weight(5e-22) # Forces are in Newtons, and typical values are ~10^5, 10^6 Newtons
...
Jforce = sum([LpCurveForce(c, coils_TF, regularization_rect(a_list[i], b_list[i]), p=2, threshold=1e5 * 40) for i, c in enumerate(base_coils_TF)])
Jforce2 = sum([SquaredMeanForce(c, coils_TF) for c in (base_coils_TF)])
Jtorque = sum([LpCurveTorque(c, coils_TF, regularization_rect(a_list[i], b_list[i]), p=2, threshold=1e5 * 40) for i, c in enumerate(base_coils_TF)])
