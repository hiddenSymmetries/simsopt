...
base_curves_finite_build = sum([
    create_multifilament_grid_frenet(c, numfilaments_n, numfilaments_b, gapsize_n, gapsize_b, rotation_order=rot_order) for c in base_curves], [])
...
coils_exp = [Coil(c, curr) for (c, curr) in zip(base_curves_finite_build, base_currents_finite_build)]
...
Jstrain_list = [strain_opt(c, width=width, scale=scale) for c in base_curves_finite_build]
Jstrain = sum(QuadraticPenalty(Jstrain_list[i], Jstrain_list[i].J()) for i in range(len(base_curves_finite_build)))
...
def fun(dofs):
    ...
    #print(f"J={J:.3e}, Jflux={jf:.3e}, sqrt(Jflux)/Mean(|B|)={np.sqrt(jf)/mean_AbsB:.3e}, CoilLengths=[{cl_string}], [{kap_string}], {strain_string:.3e}, ||∇J||={np.linalg.norm(grad):.3e}")
