...
OUT_DIR = "./passive_coils_Nscan/"
if os.path.exists(OUT_DIR):
    shutil.rmtree(OUT_DIR)
os.makedirs(OUT_DIR, exist_ok=True)

# Loop over the number of PSCs in the array
for Nx in Nvals:
    # Nx = 5
    Ny = Nx
    Nz = Nx
    ...
    rad_pscs.append(base_curves[0].x[0])
    ...
    for i in range(1, n_saves + 1):
        ...
        btot.set_points(s.gamma().reshape((-1, 3)))
        bn_mean = np.mean(np.abs(np.sum(btot.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)))
        mean_Bn.append(bn_mean)
        mean_Bn_over_B.append(bn_mean / np.mean(btot.AbsB()))
        print('Mean ⟨B·n⟩ = ', mean_Bn)
        print('Mean ⟨B·n⟩/⟨B⟩ = ', mean_Bn_over_B)
...
print(num_pscs, rad_pscs, mean_Bn, mean_Bn_over_B)
color = 'tab:red'
fig, ax1 = plt.subplots()
ax1.grid()
# plt.semilogy(num_pscs, mean_Bn, 'ko')
ax1.plot(num_pscs, mean_Bn_over_B, 'ko')
ax1.set_xlabel('# of PSCs')
ax1.set_ylabel('⟨B·n⟩/⟨B⟩')
ax1.tick_params(axis='y', labelcolor=color)
# ax1.legend(['⟨B·n⟩/⟨B⟩'])
ax2 = ax1.secondary_xaxis('top')
color = 'tab:red'
# plt.semilogy(rad_pscs, mean_Bn_over_B, 'ro')
ax2.set_xlabel('PSC coil radius (m)', color=color)
ax2.set_ticks(num_pscs)
ax2.set_xticklabels(["{0:.2f}".format(rad_pscs[i]) for i in range(len(rad_pscs))])
ax2.tick_params(axis='x', labelcolor=color)
# plt.legend(['⟨B·n⟩', '⟨B·n⟩/⟨B⟩'])
plt.show()
