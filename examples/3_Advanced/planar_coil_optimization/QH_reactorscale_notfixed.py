...
#         40, s.nfp, stellsym=True, 
#         R0=s.get_rc(0, 0), R1=40, order=3, numquadpoints=256,
#         jax_flag=True,
# )
# import warnings
...
#     OUT_DIR + "curves_0", 
#     close=True, 
#     extra_point_data=pointData_forces_torques(coils, coils_TF, np.ones(len(coils)) * aa, np.ones(len(coils)) * bb, np.ones(len(coils)) * nturns),
#     I=currents,
#     NetForces=coil_net_forces(coils, coils_TF, regularization_rect(np.ones(len(coils)) * aa, np.ones(len(coils)) * bb), np.ones(len(coils)) * nturns),
#     NetTorques=coil_net_torques(coils, coils_TF, regularization_rect(np.ones(len(coils)) * aa, np.ones(len(coils)) * bb), np.ones(len(coils)) * nturns)
# )
...
#     p=2, threshold=1e8) for i, c in enumerate(base_coils_TF)])
# Jforce2 = LpCurveForce2(coils, coils_TF, p=2, threshold=1e8)
# Jforce = sum([SquaredMeanForce(c, coils_TF) for c in (base_coils_TF)])
...
# Jtorque2 = sum([SquaredMeanTorque(c, coils_TF) for c in (base_coils_TF)])

# Jtorque2 = sum([SquaredMeanTorque(c, coils_TF) for c in (base_coils_TF)])


# Jtorque = SquaredMeanTorque2(coils, coils_TF) # [SquaredMeanForce2(c, coils) for c in (base_coils)]
# Jtorque = [SquaredMeanTorque(c, coils_TF) for c in (base_coils_TF)]
for c in coils_TF:
    # print(c._children)
    c._children = set()  # set(list(c._children)[0:1])
