...
def parse_bo_log(filename):
    ...
    pattern = re.compile(
        r'(\d+)\)\s*Best value:\s*'
        r'([-\deE+.]+)\s*,\s*TR length:\s*([-\deE+.]+)\s*'
        r'at point:\s*tensor\(\[(.*?)\]\s*,\s*dtype=torch\.float64\)\s*'
        r'length scales:\s*tensor\(\[(.*?)\]\s*,\s*dtype=torch\.float64\)',
        re.DOTALL
    )

    matches = pattern.findall(text)

    for iter_str, best_str, _, dofs_str, ls_str in matches:
        iteration = int(iter_str)
        best_val = float(best_str)

        dofs_str = dofs_str.replace("\n", " ")
        ls_str = ls_str.replace("\n", " ")

        dofs = np.fromstring(dofs_str.replace(",", " "), sep=" ")
        ls_vals = np.fromstring(ls_str.replace(",", " "), sep=" ")

        if dofs.size == 0 or ls_vals.size == 0:
            continue
...
def plot_one_case(fpath, subfig):
    dofs, iter_ids, best_vals, lengthscales = parse_bo_log(fpath)
    
    #gs = GridSpec(2, 3, width_ratios=[1, 1,3], hspace=0.2, figure=subfig)
    gs = subfig.add_gridspec(1, 2, hspace=0.3, wspace=0.15)

    ax_top = subfig.add_subplot(gs[0, 0])
    ax_bot = subfig.add_subplot(gs[0, 1])
    ...
    ax_top.set_xlabel("Function evaluations")
    #ax_top.set_ylabel("Target function value")
    ax_top.set_title("Target function value by evaluation number")
    ...
    ax_bot.set_xlabel("Function evaluations")
    ax_bot.set_ylabel("Length scale")
    ax_bot.set_title("GP length scales by evaluation number")
    ax_bot.grid(True, which='both', alpha=0.3)
    ax_bot.legend()

if __name__ == "__main__":
    fpath_list=['turbo.out']
    fig = plt.figure(figsize=(18, 3))

    plot_one_case(fpath_list[0], fig)

    #plt.show()
    plt.savefig('turbo_center_prog_only.pdf', bbox_inches='tight')
