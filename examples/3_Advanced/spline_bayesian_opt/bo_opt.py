...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        count = 0
        X_sobol = SobolEngine(
            dimension=len(optimizer.lb), scramble=True, seed=0
        )
        while count < n_init:  # len(initial_y) < n_init:
            X = X_sobol.draw(batch_size).to(
                dtype=optimizer.dtype, device=optimizer.device
            )
            # print(f'X: {X}')
            initial_X.append(X)
            stop = [0]
            new_y = parallel_batch_target(
                X, optimizer.spline_kwargs, optimizer.lb, optimizer.ub, stop
            )
