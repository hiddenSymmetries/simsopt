...
def pointData_forces_torques(coils, allcoils, aprimes, bprimes):
    ...
    for i, c in enumerate(coils):
        ...
        # print(B_other)
        # exit()
        forces[i, :-1, :] = coil_force(c, allcoils, regularization_rect(aprime, bprime))
        # print(i, forces[i, :-1, :])
        # bs._coils = coils
        torques[i, :-1, :] = coil_torque(c, allcoils, regularization_rect(aprime, bprime))
...
FORCE_WEIGHT = Weight(1e-16) # Forces are in Newtons, and typical values are ~10^5, 10^6 Newtons
