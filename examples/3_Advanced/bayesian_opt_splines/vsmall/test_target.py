...
def parallel_batch_target(candidates, spline_kwargs, lb, ub, stopp):
    ...
    if stopp[0]==0:
        ...
        if rank==0:
            print(f'{timestamp} batch complete. ')
...
def target(X, spline_kwargs, lb, ub):
    ...
    try:
        ...
        print(f'{timestamp} Failed with exception {e}, appending invalid penalty')
        surf.plot()
        plt.savefig(f'fails/fail_{timestamp}_splines.png')
        plt.close()
        with open(f'fails/fail_{timestamp}.txt', "w") as f:
            json.dump(surf.x.tolist(), f)
        rz_surf = surf.to_RZFourier(
            nu=64,
            nv=64,
            nv_interp=128,
        ...
        rz_surf.plot(show=False)
        plt.savefig(f'fails/fail_{timestamp}_ft.png')
        plt.close()
