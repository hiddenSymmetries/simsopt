...
def parallel_batch_target(candidates, spline_kwargs, lb, ub, stopp):
    ...
    if stopp[0]==0:
        ...
        if rank==0:
            ...
            return torch.Tensor(Y_cand).reshape(-1, 1), torch.Tensor(Yvar_cand).reshape(-1, 1)
...
def target(X, spline_kwargs, lb, ub):
    ...
    try:
        ...
        vol = np.sqrt(10)*(vmec.volume() - 0.4)
        residuals = np.array([ar_penalty] + [iota_edge_penalty] + [iota_axis_penalty] + [vol])
        ...
        return np.maximum(resnn, INVALID_PENALTY), default_fev_var
