...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        while count < n_init:#len(initial_y) < n_init:
            ...
            initial_X.append(X)
            stop=[0]
            new_y, new_yvar = parallel_batch_target(X, optimizer.spline_kwargs, optimizer.lb, optimizer.ub, stop)
        ...
        optimizer.Y_var = torch.Tensor(np.array(initial_yvar), device=optimizer.device).reshape(-1, 1).to(torch.double)
        ...
        while i < max_iter:
            ...
            new_y, new_yvar = parallel_batch_target(new_x, spline_kwargs, lb, ub, stop)
            print(f'{i}/{max_iter}:\n')
            for k, _ in enumerate(new_x[:, 0]):
                # print(f'candidate {k}: {from_unit_cube(new_x[k, :], lb, ub)}')
                print(f'f(candidate{k}): {new_y[k, :]}')
            optimizer.tell(new_x, new_y, new_yvar, lb, ub)
            sys.stdout.flush()
            i += 1
        stop=[1]
