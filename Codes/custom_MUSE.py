...
#most updated, all issues with outputs should be resolved
...
out_dir = Path("output_CUSTOM_permanent_magnet_GPMO_MUSE")
...
if save_plots:
    ...
    for k in range(m_history.shape[-1]):
        if not np.allclose(m_history[:, :, k], np.zeros(m_history[:, :, k].shape)): 
...
print(pm_opt.m,pm_opt.m.shape)
print(pm_opt.A_obj,pm_opt.A_obj.shape)

D = len(pm_opt.m)/3
dphi = quadpoints_phi[2] - quadpoints_phi[1]
dtheta = quadpoints_theta[2] - quadpoints_theta[1]
A = np.zeros(())
for i in range(0,nphi):
    ri = np.array([quadpoints_phi[i],quadpoints_theta[i]])
    gi = mu0/(4*np.pi) * ((3*s.unitnormal@ri)*ri/np.linalg.norm(ri)**5 - s.unitnormal/np.linalg.norm(ri)**3) * np.sqrt(dphi*dtheta*np.linalg.norm(s.unitnormal))
