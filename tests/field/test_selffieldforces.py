...
class CoilForcesTest(unittest.TestCase):
    ...
    def test_circular_coil(self):
        ...
        curve = create_equally_spaced_curves(1, 1, False, 0, 1)[0]
        current = Current(I)
        ...
        hoop_force_analytical = np.linalg.norm(constants.mu_0 *
                                               I / 4 / np.pi / R1 * (np.log(8 * R1 / a) - 3 / 4) * n, axis=1)

        hoop_force_test = np.linalg.norm(field_on_coils(coil, a), axis=1)

        np.testing.assert_array_equal(hoop_force_test, hoop_force_analytical)
    ...
    def test_forces_taylor_test(self):
        """Try whether dJ matches finiite differences of J"""
        nfp = 4
        ncoils = 4
        I = 1
