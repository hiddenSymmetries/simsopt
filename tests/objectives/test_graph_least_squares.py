...
class LeastSquaresProblemTests(unittest.TestCase):
    def test_single_value_opt_in(self):
        ...
        iden.x = [0]
        term1 = LeastSquaresProblem.from_sigma(3, 2, iden)
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError):
            lst = LeastSquaresProblem.from_sigma(3, 0, iden)

        # Weight cannot be negative
        with self.assertRaises(ValueError):
            lst = LeastSquaresProblem(3, -1.0, iden)
    ...
    def test_multiple_funcs_single_input(self):
        ...
        lsp = LeastSquaresProblem.from_sigma([3, -4], [2, 5], [iden1, iden2])
    ...
    def test_least_squares_combination(self):
        iden1 = Identity()
        iden2 = Identity()
        term1 = LeastSquaresProblem.from_sigma(3, 2, iden1)
        term2 = LeastSquaresProblem.from_sigma(-4, 5, iden2)
        lsp = term1 + term2
        iden1.x = [10]
        self.assertAlmostEqual(np.abs(lsp.residuals()[0]), 3.5)
        self.assertAlmostEqual(np.abs(lsp.residuals()[1]), 0.8)
