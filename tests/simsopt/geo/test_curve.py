...
def test_curve_second_derivative(curvetype, rotated):
    h = 0.1
    epss = [0.5**i for i in range(10, 15)] 
    x = np.asarray([0.6] + [0.6 + eps for eps in epss])
    curve = get_curve(curvetype, rotated, x)
...
def test_curve_third_derivative(curvetype, rotated):
    h = 0.1
    epss = [0.5**i for i in range(10, 15)] 
    x = np.asarray([0.6] + [0.6 + eps for eps in epss])
    curve = get_curve(curvetype, rotated, x)
...
def test_coil_dof_numbering(curvetype, rotated):
    cfc = get_curve(curvetype, rotated)
    coeffs = cfc.get_dofs()
    cfc.set_dofs(coeffs)
    assert(np.allclose(coeffs, cfc.get_dofs()))

def test_coil_coefficient_derivative(curvetype, rotated):
    cfc = get_curve(curvetype, rotated)
    coeffs = cfc.get_dofs()
...
def test_coil_kappa_derivative(curvetype, rotated):
...
def test_curve_kappa_first_derivative(curvetype, rotated):
    h = 0.1
...
def test_curve_incremental_arclength_derivative(curvetype, rotated):
    # This implicitly also tests the higher order derivatives of gamma as these
    # are needed to compute the derivative of the curvature.
...
def test_curve_kappa_derivative(curvetype, rotated):
    cfc = get_curve(curvetype, rotated)
    coeffs = cfc.get_dofs()
    def f(dofs):
        cfc.set_dofs(dofs)
...
def test_curve_torsion_derivative(curvetype, rotated):
    cfc = get_curve(curvetype, rotated)
    coeffs = cfc.get_dofs()
    def f(dofs):
        cfc.set_dofs(dofs)
...
def test_magnetic_axis_frenet_frame(curvetype, rotated):
    ma = get_curve(curvetype, rotated)
...
def test_magnetic_axis_frenet_frame_derivative(curvetype, rotated):
    ma = get_curve(curvetype, rotated)
    coeffs = ma.get_dofs()
    def f(dofs):
        ma.set_dofs(dofs)
...
def test_magnetic_axis_dkappa_by_dphi_derivative(curvetype, rotated):
    ma = get_curve(curvetype, rotated)
    coeffs = ma.get_dofs()
    def f(dofs):
        ma.set_dofs(dofs)
