...
#        assert rel_err < 1e-5
    ...
    def test_aspect_ratio_compare_with_cross_sectional_computation_halfperiod(self):
        """
        This test validates the VMEC aspect ratio computation in the Surface class by 
        comparing with an approximation based on cross section computations.
        """
        ...
        #s1 = SurfaceXYZFourier(mpol=s.mpol, ntor=s.ntor, nfp=s.nfp, stellsym=s.stellsym,
        #                      quadpoints_phi=phis, quadpoints_theta=thetas)
        #s1.set_dofs(s.get_dofs())
 


#        nphi = 10
#        ntheta = 10
#        phis = np.linspace(0, 1./3., nphi, endpoint=False)
#        thetas = np.linspace(0, 1, ntheta, endpoint=False)
#        s2 = SurfaceXYZFourier(mpol=s.mpol, ntor=s.ntor, nfp=s.nfp, stellsym=s.stellsym,
#                              quadpoints_phi=phis, quadpoints_theta=thetas)
#        s2.set_dofs(s.get_dofs())
#        import ipdb;ipdb.set_trace()
        ...
        phis1 = np.linspace(0, 1/6, nphi, endpoint=False)
        thetas1 = np.linspace(0, 1, ntheta, endpoint=False)
        s2 = SurfaceXYZFourier(mpol=s.mpol, ntor=s.ntor, nfp=s.nfp, stellsym=s.stellsym,
                              quadpoints_phi=phis1, quadpoints_theta=thetas1)
        s2.set_dofs(s.get_dofs())
        ...
        phis2 = np.linspace(0, 1/3, nphi, endpoint=False)
        thetas2 = np.linspace(0, 1, ntheta, endpoint=False)
        s3 = SurfaceXYZFourier(mpol=s.mpol, ntor=s.ntor, nfp=s.nfp, stellsym=s.stellsym,
                              quadpoints_phi=phis2, quadpoints_theta=thetas2)
        s3.set_dofs(s.get_dofs())
        ...
        #
        ## Plot the surface
        #ax.plot_surface(X, Y, np.sum(s3.gamma()*s3.normal(), axis=-1))
        #
        #ax.set_xlabel('X')
        #ax.set_ylabel('Y')
        #ax.set_zlabel('Z')
        ...
        print(np.abs(s2.volume()+2.6326782330973075), np.abs(s3.volume()+2.6326782330973075) )
        print(np.abs(s2.aspect_ratio()-4.73398998473114) , np.abs(s3.aspect_ratio() - 4.73398998473114) )
        import ipdb;ipdb.set_trace()

        #vpr = s.quadpoints_phi.size + 100
        #tr = s.quadpoints_theta.size + 100
        #cs_area = np.zeros((vpr,))
        ...
        #AR = s.aspect_ratio()
        #
        #AR_shp = shp.aspect_ratio()

        #rel_err = np.abs(AR-AR_cs) / AR
        #print(AR, AR_shp, AR_cs)
        #print("AR rel error is:", rel_err)
