...
class QfmSurfaceTests(unittest.TestCase):
    ...
    def test_qfm_constrained_jacobian(self):
        """
        ...
        for surfacetype in surfacetypes_list:
            for stellsym in stellsym_list:
                with self.subTest(surfacetype=surfacetype, stellsym=stellsym):
                    self.subtest_qfm_constrained_jacobian(surfacetype, stellsym)

    def subtest_qfm_constrained_jacobian(self, surfacetype, stellsym):
        np.random.seed(1)
        coils, currents, ma = get_ncsx_data()
        stellarator = CoilCollection(coils, currents, 3, True)
        ...
        res0, dres0 = qfm_surface.qfm_exact_constraints(xl, derivatives=1)
        ...
        print("################################################################################")
        for eps in epsilons:
            res1 = qfm_surface.qfm_exact_constraints(xl + eps*h, derivatives=0)
    ...
    def subtest_qfm_surface_optimization_convergence(self, surfacetype,
        ...
        assert res['success']
        assert np.linalg.norm(res['fun']) < 1e-5
