...
class SurfaceHennebergTests(unittest.TestCase):
    ...
    def test_set_get_dofs(self):
        ...
        for mmax in range(1, 4):
            for nmax in range(4):
                surf = SurfaceHenneberg(nfp=1, alpha_times_2 = 1, mmax=mmax, nmax=nmax)
                self.assertEqual(len(surf.names), len(surf.get_dofs()))
                
                # R0nH has nmax+1
                # Z0nH has nmax
                # bn has nmax + 1
                # rhomn for m=0 has nmax + 1
    ...
    def test_set_get_rhomn(self):
        ...
        for mmax in range(1, 4):
            for nmax in range(4):
                surf = SurfaceHenneberg(nfp=2, alpha_times_2 = -1, mmax=mmax, nmax=nmax)
    ...
    def test_fixed_range(self):
        ...
        np.testing.assert_equal(surf.fixed, [True]*13)
        surf.fixed_range(20, 20, False)
        np.testing.assert_equal(surf.fixed, [False]*13)
        surf.fixed_range(0, 0, True)
        np.testing.assert_equal(surf.fixed, [True, False, False, False, False, True, False,
                                             False, False, False, False, False, False])
        surf.fixed_range(1, 0, True)
        np.testing.assert_equal(surf.fixed, [True, False, False, True, False, True, False,
        ...
        np.testing.assert_equal(surf.fixed, [False, False, False, True, True, False, False,
                                             True, True, True, True, True, True])
        surf.fixed_range(1, 1, False)
        np.testing.assert_equal(surf.fixed, [False, False, False, False, False, False, False,
    ...
    def test_axisymm(self):
        ...
        for nfp in range(1, 3):
            for alpha_times_2 in [-1, 1]:
                for mmax in range(1, 3):
                    for nmax in range(3):
                        surfH = SurfaceHenneberg(nfp=nfp, alpha_times_2=alpha_times_2, mmax=mmax, nmax=nmax)
                        surfH.R0nH[0] = R0
                        surfH.bn[0] = a
