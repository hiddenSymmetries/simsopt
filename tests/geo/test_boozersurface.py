...
#        assert np.max(np.abs(r0[-2:])) < 1e-6
    ...
    def test_dexactresidual_dcoils_vjp(self):
        ...
        coils, currents, ma = get_ncsx_data(ppp=1)
        stellarator = CoilCollection(coils, currents, 3, True)
        bs = BiotSavart(stellarator.coils, stellarator.currents)
        ...
        coeffs = np.concatenate( (stellarator.get_currents()/current_fak, stellarator.get_dofs()) )
        
        idx_curr = np.arange(0,3)
        idx_coil = np.arange(3, coeffs.size)

        def f(x):
            stellarator.set_currents(x[idx_curr]*current_fak)
            stellarator.set_dofs(x[idx_coil])
            for coil, curr in zip(booz_surf.bs.coils_optim, stellarator.currents):
        ...
        lm1_dg_dcurr = stellarator.reduce_current_derivatives(lm1_dg_dcurrents) * current_fak
        lm1_dg_dcoils = stellarator.reduce_coefficient_derivatives(lm1_dg_dcoils)
        
        lm2 = np.random.uniform(size=lm1_dg_dcoils.size+lm1_dg_dcurr.size)-0.5
        fd_exact = np.dot( np.concatenate( (lm1_dg_dcurr, lm1_dg_dcoils) ), lm2)
        ...
        for i in range(10, 40):
            eps = 0.5**i
            fd = weights[0] * f(coeffs + eps * shifts[0] * lm2)
            for i in range(1, len(shifts)):
                fd += weights[i] * f(coeffs + shifts[i] * eps * lm2)
            err = abs(np.dot(fd/eps, lm1[:-1]) - fd_exact)
            print(f"eps: {eps:.6e}, deriv: {fd_exact:.6e}, err: {err:.6e}, rel. err:{err/np.linalg.norm(fd_exact):.6e}")
    ...
    def test_dlsqgrad_dcoils_vjp(self):
        ...
        lm1_dg_dcurr = stellarator.reduce_current_derivatives(lm1_dg_dcoils[1]) * current_fak
...
#        print("################################################################################")
        ...
        for i in range(10, 40):
            ...
            for i in range(1, len(shifts)):
                fd += weights[i] * f(coeffs + shifts[i] * eps * lm2)
            err = abs(np.dot(fd/eps, lm1) - fd_exact)
            print(f"eps: {eps:.6e}, deriv: {fd_exact:.6e}, err: {err:.6e}, rel. err:{err/np.linalg.norm(fd_exact):.6e}")
